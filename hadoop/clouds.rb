...
%w{andand}.each do |gemname|
  begin
    require gemname
  rescue LoadError
    puts "This clouds.rb requires '#{gemname}': gem install #{gemname}"
    exit
  end
end
...
<Directory /var/www/>
  ...
  after_all_loaded do
    ...
    clouds[:hadoop_slave].andand.run_in_context do
      hadoop.perform_just_in_time_operations
      ganglia.perform_after_all_loaded_for_slave
    end

    clouds[:hadoop_master].andand.run_in_context do
      hadoop.perform_just_in_time_operations
